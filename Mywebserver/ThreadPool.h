...
//:thread_number(thread_num), threads(NULL), m_stop(false) {
{
        threads = NULL;
    ...
    for( int i = 0; i < thread_number; i++ ) {
        ...
        // 1) 类的静态对象
        // 2) 将类的对象作为参数传给静态函数
        // 这里通过第二种方法 将线程参数设置为this指针然后在worker函数中获取该指针并调用其动态方法run实现
        if( pthread_create( &threads[i], NULL, worker, this ) ) {  // 成功返回0
        printf("creat %dth thread\n",i);//失败了看是第几个线程创建失败的  
        ...
        //一个没有被分离的线程，在退出时，系统会保留它的虚拟内存，包括他们的堆栈和其他系统资源，这种线程被称为“僵尸线程”。创建线程时默认是非分离的，或者称为可连接的（joinable）。
